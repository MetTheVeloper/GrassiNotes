...
نسخهٔ 2.2.1 چیدمان دیداری RTL را برای پاراگراف عادی، Bullet و Numbering
اصلاح می‌کند. `Ctrl+R` و `Ctrl+L` فقط تراز را تغییر می‌دهند و جهت متن را
دست‌نخورده نگه می‌دارند. تورفتگی Bullet و Numbering یکسان شده و زوم اکنون
واقعاً روی تمام محتوای ویرایشگر اعمال و داخل فایل `.grass` ذخیره می‌شود.
گوشهٔ دکمهٔ Close نیز با قاب گرد پنجره clip می‌شود.
